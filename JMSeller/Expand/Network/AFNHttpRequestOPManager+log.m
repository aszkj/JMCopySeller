...
//    NSString *requestUrlStr = [NSString stringWithFormat:@"%@%@",JMBASEURL,requestUrl];
...
//    NSString *requestUrlName = [[self class] apiNameOfUrl:requestUrl];
    DDLogVerbose(@"<======================请求接口%@信息===========================>",requestUrl);
    DDLogVerbose(@"请求头HttpHeaders---%@",[[[[self class] sharedManager] requestSerializer] HTTPRequestHeaders]);
//    NSString *requestUrlStr = [NSString stringWithFormat:@"%@%@",JMBASEURL,requestUrl];
    DDLogVerbose(@"请求url---%@",requestUrl);
    DDLogVerbose(@"请求参数param---%@",requestParam);
    ...
    if (errorCode != 1) {
        DDLogVerbose(@"errorInfo ==> [error url = %@,code = %ld,msg --- %@]",requestUrl,responseErrorInfo.code,responseErrorInfo.localizedDescription);
        if (errorCode == -1) {
