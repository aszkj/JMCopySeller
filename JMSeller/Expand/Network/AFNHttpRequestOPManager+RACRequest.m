                ...
                subScribeResultBlock:(SubScriberResultBlock)subScribeResultBlock;
{
    ...
    return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        @strongify(self);
        [[self class] getInfoWithSubUrl:subUrl parameters:Parameters result:^(NSDictionary *result, NSError *error) {
...

{
    ...
    return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        [[self class] postWithSubUrl:subUrl parameters:Parameters result:^(NSDictionary *result, NSError *error) {
               if (subScribeResultBlock) {
                   subScribeResultBlock(result,error,subscriber);
               }
